...
const (
	agent_url = "https://raw.githubusercontent.com/petems/puppet-install-shell/master/install_puppet_5_agent.sh"
...
func (p *Provisioner) RunPuppetAgent(o terraform.UIOutput, comm communicator.Communicator) error {
	...
	err := p.runCommand(o, comm, fmt.Sprintf("'/opt/puppetlabs/bin/puppet resource host puppet ensure=present ip=%s ;/opt/puppetlabs/bin/puppet agent --enable; /opt/puppetlabs/bin/puppet agent --onetime --verbose --no-daemonize --no-usecacheonfailure --no-splay --show_diff'", p.MasterIP))
	if err != nil {
		return err
	}
	return nil
